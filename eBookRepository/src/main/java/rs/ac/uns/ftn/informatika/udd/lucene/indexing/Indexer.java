package rs.ac.uns.ftn.informatika.udd.lucene.indexing;
...
public class Indexer {
	...
	public boolean add(IndexUnit unit) {
		IndexQuery indexQuery = new IndexQueryBuilder().withObject(unit).withId(unit.getFilename()).build();
		...
		try {
			...
			for (File newFile : files) {
				if (newFile.isFile()) {
					...
					indexQueries.add(new IndexQueryBuilder().withObject(handler.getIndexUnit(newFile))
							.withId(handler.getIndexUnit(newFile).getFilename()).build());
